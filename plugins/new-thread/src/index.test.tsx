...
it('registers the new conversation page as the root index route', async () => {
  ...
  for (const module of [i18n, renderer, router, layout, theme, conversation, newThread]) {
    const fiber = ctx.plugin(module);
    fibers.push(fiber);
    await fiber.await();
  }
  expect(ctx.routes.snapshot().find(route => route.id === 'new-thread')).toMatchObject({ index: true, parentId: 'conversation' });
